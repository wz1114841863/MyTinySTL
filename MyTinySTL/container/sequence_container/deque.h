...
namespace mystl {
    ...
    void deque<T>::emplace_back(Args &&...args) {
        if (end_.cur != end_.last - 1) {  // 这里是last - 1不是last
            data_allocator::construct(end_.cur, mystl::forward<Args>(args)...);
            ++end_.cur;
        }else {
            require_capacity(1, false);  // 先扩充，填充完最后一个后，移动last
            data_allocator::construct(end_.cur, mystl::forward<Args>(args)...);
            ++end_;
        }
    }
    ...
    typename deque<T>::iterator deque<T>::insert_aux(iterator position, Args&& ...args) {
        ...
        if (elems_before < (size() / 2)) {  // 在前半段插入
            ...
            auto pos = position;
            ++pos;
            mystl::copy(front2, pos, front1);  // 往前copy，腾出pos指向的空间
        }else {  // 在后半段插入
            emplace_back(back());
            auto back1 = end_;
            --back1;

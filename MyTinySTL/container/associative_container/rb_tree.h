 ...
 * 2. 根节点为黑色
 * 3. 如果节点为红，子节点必须为黑色
 * 4. 任一节点至nullptr(树尾端）的任何路径，所含之黑节点数必须相同
 */
/*
 * 根据规则四，新增节点必须为红
 * 根据规则三，新增节点之父节点必须为黑
 * 当插入时未满足条件，说明需要调整颜色和旋转树形
 */
...
namespace mystl {
    ...
    template <typename T> struct rb_tree_base;
    ...
    // 如果是set，T为某一类型，如果是map，T为pair<first, second>
    template <typename T>
    ...
    struct rb_tree_iterator_base :public mystl::iterator<mystl::bidirectional_iterator_tag, T> {
        ...
        void inc() {
            if (node->right != nullptr) {
                node = rb_tree_min(node->right);  // 右子树的最左节点
            }else {
                auto y = node->parent;  // 找出父节点
                while (y->right == node) {  // 现行节点本身就是右节点，一直上溯
                    node = y;
                    y = y->parent;
                }
    ...
    //         然后令祖父节点为当前节点，继续处理
    // case 4: 父节点为红，叔叔节点为 nullptr 或黑色，父节点为左（右）孩子，当前节点为右（左）孩子，
    //         让父节点成为当前节点，再以当前节点为支点左（右）旋
    // case 5: 父节点为红，叔叔节点为 nullptr 或黑色，父节点为左（右）孩子，当前节点为左（右）孩子，
        ...
        while (x != root && rb_tree_is_red(x->parent)) {
            if (rb_tree_is_lchild(x->parent)) {  // 如果父节点是左子节点
                ...
                if (uncle != nullptr && rb_tree_is_red(uncle) {
                    // case 3: 父节点和叔叔节点都为红, 祖父节点必黑
                    rb_tree_set_black(x->parent);
                    rb_tree_set_black(uncle);
                    x = x->parent->parent;
                    rb_tree_set_red(x);  // 返回后还要继续进行循环
                }else {  // 无叔叔节点或叔叔节点为黑
    ...
    // TODO: 尚未理解
    // 删除节点后使 rb tree 重新平衡
    // 参数一为要删除的节点，参数二为根节点，参数三为最小节点，参数四为最大节点
    template <typename NodePtr>
    ...
    public:
        ...
        // 容量相关操作
        bool empty() const noexceptt { return node_count_ == 0; }
        ...
        // 插入删除相关操作
        // emplace
        // emplace_unique(不允许键值重复)，emplace_multi(允许键值重复)
